...
import { CategoryImagesRepository } from "../core/domain/CategoryImagesRepository";
import { getImageFolder } from "../../../utils/image";

export class AwsCategoryImagesRepository implements CategoryImagesRepository {
    private s3Client: S3Client;
    private url = process.env.AWS_CDN_URL || '';
    private bucketName = process.env.AWS_CATEGORY_BUCKET || '';
    ...
    async save(prefix: string, image: Express.Multer.File): Promise<string> {
        try {
            ...
            const fileKey = `category-images/${prefix}/${prefix}.${extension}`;
