...
import { ProductImagesRepository } from "../core/domain/ProductImagesRepository";
import { getImageFolder } from "../../../utils/image";

export class AwsProductImagesRepository implements ProductImagesRepository {
    private s3Client: S3Client;
    private url = process.env.AWS_CDN_URL || '';
    private bucketName = process.env.AWS_PRODUCT_BUCKET || '';
    ...
    async save(prefix: string, image: Express.Multer.File): Promise<string> {
        try {
            ...
            const fileKey = `product-images/${prefix}/${prefix}.${extension}`;
